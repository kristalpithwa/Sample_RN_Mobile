...
import {e, p, prettyJSON} from './util';
...
function LevelPlayRewardedVideoSection() {
  const [isRewardedVideoAvailable, setIsRewardedVideoAvailable] =
    useState(false);
  const [isRewardedVideoVisible, setIsRewardedVideoVisible] = useState(false);
  const [reservedPlacement, setReservedPlacement] = useState(null);
  ...
  const getPlacementInfo = async () => {
    const placement = await IronSource.getRewardedVideoPlacementInfo(
  ...
  const showRewardedVideo = async () => {
    ...
    if (await IronSource.isRewardedVideoAvailable()) {
      ...
      if (!isCapped) {
        ...
        showAlert('Rewarded Video Placement', [
          `${REWARDED_VIDEO_PLACEMENT} is capped`,
        ]);
      }
    } else {
  ...
  return (
    <View>
      <Text style={[styles.h2, styles.alignCenter]}>Rewarded Video</Text>
      <HighlightButton
...
const LevelPlayInterstitialAdSection = () => {
  ...
  const listener = {
    onAdLoaded: adInfo => {
      console.log('Interstitial Ad Ad loaded:', adInfo);
  ...
  return (
    <View>
      ...
      <View style={styles.horizontalSpaceBetween}>
        ...
        <HighlightButton
          buttonText={'Destroy Ad'}
          onPress={destroyAdAndCreateNew}
        />
      </View>
      <View style={styles.horizontalSpaceBetween}>
        <HighlightButton
          buttonText={'Pause Auto Refresh'}
          onPress={pauseAutoRefresh}
        />
        <HighlightButton
          buttonText={'Resume Auto Refresh'}
          onPress={resumeAutoRefresh}
        />
      </View>
      <LevelPlayBannerAdView
        ref={bannerAdViewRef}
...
const LevelPlayNativeAdViewSection = () => {
  ...
  return (
    <View>
      <Text style={[styles.h2, styles.alignCenter]}>Native Ad</Text>
      <HighlightButton buttonText={'Load Native Ad'} onPress={loadAd} />
      <HighlightButton
        buttonText={'Destroy Native Ad'}
        onPress={destroyAdAndCreateNew}
      />
      {nativeAd && (
